...
Changes:	
	...
	July 16 2018	- Disabled AutoLocalize for generated text labels
			...
			TODO:
				- exit animations <--- might do this later on ~ sholjuxt_lmao
				- emphasis animations <---- what???? ~ sholjuxt_lmao
...
richText.ColorShortcuts.Red = Color3.new(1, 0, 0)
...
richText.ColorShortcuts.SSCRed = Color3.new(1, 0.592157, 0.6)
richText.ColorShortcuts.OWPurple = Color3.new(0.65098, 0.227451, 1)
richText.ColorShortcuts.DeltaOrange = Color3.new(1, 0.356863, 0.160784)
richText.ColorShortcuts.StevPink = Color3.new(1, 0.54902, 0.878431)
richText.ColorShortcuts.StevOutlinePink = Color3.new(1, 0.12549, 0.517647)
...
defaults.UIStroke = true
defaults.Thickness = 1.96
...
function animationStyles.Appear(character, Alpha, properties)
	character.Visible = true
	local UIStroke = character:FindFirstChild("UIStroke"):: UIStroke
	if UIStroke then UIStroke.Transparency = properties.TextStrokeTransparency end
end
...
function animationStyles.Fade(character, animateAlpha, properties)
	...
	local UIStroke = character:FindFirstChild("UIStroke"):: UIStroke
	if UIStroke then
		UIStroke.Transparency = 1 - (animateAlpha * (1 - properties.TextStrokeTransparency))
	end
...
function animationStyles.AboveAppear(character, animateAlpha, properties)
	character.Visible = true
	local UIStroke = character:FindFirstChild("UIStroke"):: UIStroke
	if UIStroke then
		UIStroke.Transparency = 1 - (animateAlpha * (1 - properties.TextStrokeTransparency))
	...
	character.Position = properties.InitialPosition + UDim2.new(0, 0, 0, -(math.sin(animateAlpha * 2.4) * Amplitude / 2))
end
...
function animationStyles.SideWiggle(character, animateAlpha, properties)
	character.Visible = true
	local UIStroke = character:FindFirstChild("UIStroke"):: UIStroke
	if UIStroke then UIStroke.Transparency = properties.TextStrokeTransparency end
	local amplitude = properties.InitialSize.X.Offset * (1 - animateAlpha) * properties.AnimateStyleAmplitude
	character.Position = properties.InitialPosition + UDim2.new(0, math.sin(animateAlpha * math.pi * 2 * properties.AnimateStyleNumPeriods) * amplitude / 2, 0, 0)
end
...
function richText:New(frame: Frame, text: string, startingProperties, allowOverflow, prevTextObject)
	for _, v in pairs(frame:GetChildren()) do
		if v:GetAttribute("NoDelete") then continue end
		v:Destroy()
	end
	if allowOverflow == nil then allowOverflow = true end

	local properties = {
		UIStroke = defaults.UIStroke,
	...
	function applyProperty(name, value, frame)
		...
		for _, label in pairs(frame and {frame} or {textLabel, imageLabel}) do
			...
			elseif name:match("UIStroke") and label:IsA("TextLabel") then
				local UIStroke = Instance.new("UIStroke"); UIStroke.Parent = label
				UIStroke.Transparency = label.TextStrokeTransparency; UIStroke.Color = label.TextStrokeColor3
	...
	----- Chat voice -----
	local function PlaySound(Sound: Sound, PlaceToPlay: Instance | CFrame)
		...
		if typeof(PlaceToPlay) == "CFrame" then
			...
			NewPlace.CanQuery = false
			NewPlace.CanTouch = false
	...
	local function AddVoice()
		local CurrentVoice = properties.Voice or nil
		local VoiceParent = properties.VoiceParent or nil
		if CurrentVoice and VoiceParent then PlaySound(CurrentVoice, VoiceParent) end
	end
	...
	function printText(text: string)
		...
		if newTextLabel:FindFirstChild("UIStroke") then newTextLabel:FindFirstChild("UIStroke"):Destroy() end
		...
		for first, last in utf8.graphemes(text) do
			...
			characterLabel:SetAttribute("Text", character)
			characterLabel:SetAttribute("OriginalPos", characterLabel.Position)

			local UIGradient = Instance.new("UIGradient"); UIGradient.Parent = characterLabel
			UIGradient.Color = ColorSequence.new(Color3.new(0.545098, 0.545098, 0.545098), Color3.new(1, 1, 1))
			UIGradient.Rotation = -90
			--applyStroke(characterLabel)
	...
	function applyStroke(value, label)
		if value then
			local UIStroke = Instance.new("UIStroke"); UIStroke.Parent = label
			UIStroke.Transparency = properties.TextStrokeTransparency; UIStroke.Color = getColorFromString(properties.TextStrokeColor3)
			UIStroke.Thickness = properties.Thickness
	...
	local function updateAnimations()
		if allTextReached and #animateQueue == 0 or animationDone or frame.Parent and frame.Parent.Name:match("Removing") or frame.Name:match("Removing") then
			animationDone = true
		...
		for i = #animateQueue, 1, -1 do
			...
			if animateAlpha >= 1 then table.remove(animateQueue, i) end
		end
	end
	...
	local function setGroupVisible(frame, visible)
		...
		for _, v in frame:GetChildren() do
			if v:IsA("Frame") or v:IsA("TextLabel") then v.Visible = visible; elseif v:IsA("UIStroke") then v.Enabled = true end	
			if v:IsA("TextLabel") then
				local Text = v:: TextLabel

				local fontFace = Text.FontFace
				fontFace.Weight = Enum.FontWeight.ExtraBold

				Text.FontFace = fontFace
				if v:FindFirstChild("UIStroke") then v:FindFirstChild("UIStroke").Color = v.TextStrokeColor3; v:FindFirstChild("UIStroke").Transparency = v.TextStrokeTransparency end
			end

			if visible then setFrameToDefault(v) end
		end
	...
	local function animate(waitForAnimationToFinish)
		...
		for lineNum, list in textFrames do
			for _, frame in list do setGroupVisible(frame, false) end
		end
